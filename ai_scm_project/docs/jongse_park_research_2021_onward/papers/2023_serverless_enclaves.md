...
This paper focuses on trusted serverless enclaves. Its detailed relevance comes from the way it treats AI performance as a systems problem rather than a raw arithmetic problem. The main technical themes are isolation, security overhead, and multi-tenant systems.
...
이 논문은 trusted serverless enclaves을 다룬다. 세부적으로는 isolation, security overhead, and multi-tenant systems. 같은 요소가 핵심이다. 단순히 연산량을 줄이는 것이 아니라, 실제 시스템에서 병목이 어디에서 생기는지를 찾고 그 병목을 줄이는 구조를 제안한다는 점이 중요하다.
