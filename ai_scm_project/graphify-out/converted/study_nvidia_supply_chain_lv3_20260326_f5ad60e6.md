<!-- converted from study_nvidia_supply_chain_lv3_20260326.docx -->


[Lv.3 전문가] NVIDIA GPU 공급망 & 비즈니스 모델 심화
...
# 전문가 심층 분석
이 섹션은 Level 3 전문가 과정으로, NVIDIA의 수직통합 전략 분석 (NVLink → NVSwitch → DGX Cloud)과 반독점 규제 리스크를 다룹니다.
## NVIDIA 수직통합 전략: NVLink → NVSwitch → DGX Cloud
NVIDIA는 단순 GPU 판매를 넘어 하드웨어-소프트웨어-클라우드 수직통합 구조로 진화하고 있습니다. 이는 Apple의 실리콘-iOS-앱스토어 전략과 유사한 moat 구축입니다.

수직통합 전략의 핵심 효과: 각 레이어가 아래 레이어의 수요를 창출. DGX Cloud → DGX 시스템 수요 → GPU 수요 → NVLink 수요 → CUDA 의존성 심화. 이 구조가 완성되면 NVIDIA는 단순 GPU 벤더에서 AI 인프라 플랫폼으로 전환 완료. 비교: AWS의 EC2→S3→Lambda→Bedrock 수직통합과 유사한 패턴.
## NVIDIA 반독점 리스크 분석
NVIDIA의 AI GPU 시장 지배력이 규제 당국의 시선을 끌고 있습니다. 실제 반독점 리스크를 법적 근거와 함께 분석합니다.

전문가 투자 관점: 반독점 리스크는 단기 주가 이벤트 리스크이나 사업 구조 분리(Break-up) 가능성은 낮음. 역사적 선례: Microsoft(2001년 분리 명령 → 항소 승소), Google(2024 판결 → 구제책 논의 중). NVIDIA의 수직통합이 '소비자 이익' 침해를 입증하기 어려운 구조 → 법적 방어력 강함. 모니터링: DOJ 공식 기소 여부 → 발생 시 주가 10-20% 단기 조정 가능.
...
| 레이어 | 제품 | 역할 | 경쟁 차별화 | 수익 구조 |
| --- | --- | --- | --- | --- |
| 레이어 1: GPU (칩) | H100/B200/Rubin | AI 연산의 핵심 하드웨어 | TSMC 파운드리 의존 (유일한 외부 의존), CoWoS 패키징 병목 | ~60% 영업이익률 |
| 레이어 2: 연결망 (NVLink/NVSwitch) | NVLink 5.0, NVSwitch | GPU 간 초고속 연결 (1.8TB/s) | NVIDIA 독자 설계, 경쟁사 접근 불가. NVLink Domain: 576 GPU 직접 연결 가능 | 별도 ASP 부과, 시스템 가격 +30-40% |
| 레이어 3: 시스템 (DGX/HGX) | DGX B200, HGX B200 | GPU+NVLink+InfiniBand 통합 서버 | ODM(Foxconn, Wistron)이 조립하나 NVIDIA 설계 독점 | DGX B200 판가 $500K+ |
| 레이어 4: 네트워크 (InfiniBand) | Quantum-3, ConnectX-7 | 클러스터 간 고속 네트워크 | Mellanox 2020년 $7B 인수. AI DC InfiniBand 점유율 80%+ | 연간 $6B+ 매출 (추정) |
| 레이어 5: 소프트웨어 (CUDA/cuDNN) | CUDA 12.x, TensorRT, Triton | AI 개발 표준 플랫폼 | 3,000만 개발자 설치 베이스. 무료 제공으로 전환 비용 내재화 | 간접 수익: GPU 판매 촉진 |
| 레이어 6: 클라우드 (DGX Cloud) | DGX Cloud (GCP/Oracle/Azure 통해) | NVIDIA GPU 직접 클라우드 임대 | CSP 우회한 직접 고객 접촉. $37K/월/GPU (H100 기준 추정) | 2024년 런칭, 규모화 진행 중 |
| 조사/소송 | 내용 | 현재 리스크 수준 | 법적 근거 & 반론 | 예상 전개 |
| --- | --- | --- | --- | --- |
| 미국 DOJ/FTC 조사 | 2024년 DOJ, NVIDIA 반독점 조사 소환장 발부 (보도). 혐의: 독점적 지위 남용, 고객이 AMD로 전환 시 불이익 제공 여부 | 중간 | 소환장 = 조사 착수, 기소 아님. Google 반독점 판결(검색) 선례 고려. NVIDIA는 소프트웨어를 무료 제공 → '자연스러운' lock-in 주장 | 2025-2027년 심의 지속, 기소 가능성 20-30% |
| EU 경쟁법 조사 | 유럽 Commission, AI 칩 시장 지배력 예비 조사 (2024). Run:ai 인수($700M, 2024) 심사 | 낮음~중간 | EU는 빅테크에 강경한 편 (Google, Meta 과징금 선례). Run:ai 인수는 AI 워크로드 관리 소프트웨어 → 독점 강화 우려 | 과징금 가능성 있으나 사업 구조 분리 가능성 낮음 |
| 중국 반독점 조사 | SAMR(시장감독총국) 조사 가능성. Mellanox 인수 승인 조건 (중국 시장 접근 제한 금지) 이행 여부 | 중간 | 중국 매출 제한된 상황에서 상징적 조치 가능. 지정학적 보복 수단으로 활용될 소지 | 소규모 과징금 또는 조건부 시정 조치 |
| 고객 반독점 소송 | CSP, 스타트업이 NVIDIA의 독점적 가격 책정 소송 제기 가능 | 낮음 | GPU 구매자가 독점 소송 제기 시 '제품 구매를 강요받지 않았다'는 논리 약함. 단, CUDA 무료 + 고사양 GPU 번들 판매 강요 논란 | 개별 사례 합의로 마무리 가능성 |
| 출처명 | URL | 설명 | 연도 |
| --- | --- | --- | --- |
| NVIDIA — Annual Report (10-K) FY2024 | https://investor.nvidia.com/financial-information/annual-reports/default.aspx | NVIDIA 사업 구조, 데이터센터 매출, 수출 규제 리스크, CUDA 비즈니스 모델 공시 | 2024 |
| NVIDIA Quarterly Earnings — Q4 FY2025 | https://investor.nvidia.com/financial-information/quarterly-results/default.aspx | 데이터센터 매출 $49.2B(분기), 영업이익률 60%+, Blackwell 출하 시작 확인 | 2025 |
