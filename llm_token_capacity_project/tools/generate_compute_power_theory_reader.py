"""Generate a Korean theory reader for AI compute and power allocation.

This artifact is for study, not forecasting. It explains how model owners move
from contracted power to service-level compute allocation, and how researchers
infer training/inference shares when firms do not disclose internal telemetry.
"""
...
SOURCES = [
    ("S01", "IEA, Energy and AI", "IEA", "2025", "https://www.iea.org/reports/energy-and-ai", "데이터센터 전력 수요와 AI 전력 전망의 macro baseline"),
    ("S02", "2024 United States Data Center Energy Usage Report", "Lawrence Berkeley National Laboratory / DOE", "2024-12", "https://buildings.lbl.gov/publications/2024-lbnl-data-center-energy-usage-report", "미국 데이터센터 에너지 사용, AI server 추정, water/power baseline"),
    ("S03", "Data Centers and Their Energy Consumption: Frequently Asked Questions", "Congressional Research Service", "2026", "https://www.congress.gov/crs-product/R48646", "미국 정책/전력망 관점의 데이터센터 FAQ"),
    ("S04", "Electricity Demand and Grid Impacts of AI Data Centers", "arXiv", "2025", "https://arxiv.org/abs/2509.07218", "AI 데이터센터 부하 특성과 전력망 영향 review"),
    ("S05", "Scaling Laws for Neural Language Models", "Kaplan et al., arXiv", "2020", "https://arxiv.org/abs/2001.08361", "훈련 compute, parameter, data 간 scaling law"),
    ("S06", "Training Compute-Optimal Large Language Models", "Hoffmann et al., arXiv", "2022", "https://arxiv.org/abs/2203.15556", "Chinchilla compute-optimal scaling, parameter/token allocation"),
    ("S07", "Training compute of frontier AI models grows by 4-5x per year", "Epoch AI", "2024", "https://epoch.ai/blog/training-compute-of-frontier-ai-models-grows-by-4-5x-per-year", "frontier training compute trend"),
    ("S08", "How much power will frontier AI training demand in 2030?", "Epoch AI", "2025", "https://epoch.ai/blog/power-demands-of-frontier-ai-training/", "single frontier training run의 전력 수요 추정"),
    ("S09", "Efficient Memory Management for LLM Serving with PagedAttention", "Kwon et al., arXiv", "2023", "https://arxiv.org/abs/2309.06180", "KV cache, batching, serving throughput bottleneck"),
    ("S10", "Splitwise: Efficient generative LLM inference using phase splitting", "Patel et al., arXiv", "2023", "https://arxiv.org/abs/2311.18677", "prefill/decode 분리와 power/cost 최적화"),
    ("S11", "DistServe: Disaggregating Prefill and Decoding", "Zhong et al., arXiv", "2024", "https://arxiv.org/abs/2401.09670", "TTFT/TPOT SLO와 prefill/decode resource allocation"),
    ("S12", "Google Ironwood TPU: age of inference", "Google Cloud", "2025-04-09", "https://blog.google/products/google-cloud/ironwood-tpu-age-of-inference/", "inference-oriented accelerator와 TPU serving 방향"),
    ("S13", "Cloud TPU inference documentation", "Google Cloud", "accessed 2026-05-14", "https://docs.cloud.google.com/tpu/docs/v5e-inference", "serving quota와 inference deployment 개념"),
    ("S14", "AWS Trainium", "AWS", "accessed 2026-05-14", "https://aws.amazon.com/ai/machine-learning/trainium/", "training/inference용 custom accelerator positioning"),
    ("S15", "Amazon and Anthropic expand strategic collaboration", "Amazon", "2024", "https://www.aboutamazon.com/news/company-news/amazon-invests-additional-5-billion-anthropic-ai/", "Claude, Trainium, Bedrock inference attribution 사례"),
    ("S16", "OpenAI compute infrastructure / Stargate", "OpenAI", "2025-2026", "https://openai.com/index/building-the-compute-infrastructure-for-the-intelligence-age/", "planned capacity와 model-owner compute 전략 사례"),
]
...
def md_table(rows: list[tuple[str, str, str, str, str, str]]) -> list[str]:
    out = ["| ID | 자료 | 발행자 | 날짜 | 용도 | 링크 |", "|---|---|---|---|---|---|"]
    for sid, title, publisher, dt, url, use in rows:
        out.append(f"| {sid} | {title} | {publisher} | {dt} | {use} | {url} |")
    return out
...
def build_markdown() -> str:
    lines = [
        ...
        "## 목적",
        "",
        "이 문서는 LLM token capacity simulation의 숫자를 더 현실적으로 만들기 위한 배경 이론 교재입니다. 핵심은 회사별 내부 allocation을 단정하는 것이 아니라, 공개자료와 논문을 기반으로 어떤 가정이 물리적으로, 운영적으로, 경제적으로 말이 되는지 판단하는 능력을 쌓는 것입니다.",
        "",
        "## 한 문장 요약",
        "",
        "계약 전력은 token capacity가 아니며, LLM 회사의 실제 compute 배분은 전력 인프라, accelerator cluster, scheduler, training/inference workload, 제품 SLA, business priority가 겹쳐 결정됩니다.",
        "",
    ]
    ...
    for title, paragraphs, takeaways in LONGFORM_CHAPTERS:
        lines.append(f"### {title}")
        lines.append("")
        for paragraph in paragraphs:
            lines.append(paragraph)
            lines.append("")
        lines.append("**핵심 takeaways**")
        lines.append("")
    ...
    lines.extend(
        [
            ...
            "- **Utilization:** 이론 throughput 중 실제 서비스 token으로 전환되는 평균 비율.",
            "- **SLO:** service-level objective. latency/availability 목표가 capacity reserve를 결정.",
            "",
            "## 공부 질문",
            "",
            "1. 이 회사의 발표는 planned capacity인가, operational capacity인가?",
            "2. 해당 전력은 facility power인가, IT load인가, accelerator board power인가?",
            "3. 이 capacity는 training용 cluster인가, inference serving용 cluster인가, 둘 다 가능한가?",
            "4. 서비스 traffic이 latency-sensitive인가, batchable한가?",
            "5. inference workload는 prefill-heavy인가, decode-heavy인가?",
            "6. 모델은 dense인가 MoE인가? total parameter와 active parameter가 분리되어 있는가?",
            "7. benchmark 논문이 보여주는 mechanism을 회사 production number로 오해하고 있지는 않은가?",
            "",
            "## 읽기 자료 요약",
            "",
        ]
    )
    for sid, title, publisher, dt, url, use in SOURCES:
        lines.extend(
            [
                f"### {sid}. {title}",
                "",
                ...
                f"- 링크: {url}",
                f"- 읽는 목적: {use}",
                "- 기록 방식: 원문 숫자 1-3개, 핵심 동사, 우리 모델에서 바뀔 수 있는 assumption_id를 `data/source_review_log.md`에 남깁니다.",
                "",
            ]
        )
...
def build_docx(path: Path, markdown_text: str) -> None:
    doc = Document()
    ...
    run = title.add_run("LLM Compute & Power Allocation Theory Reader")
    run.bold = True
    run.font.size = Pt(20)
    ...
    subtitle.add_run(f"생성일: {RUN_DATE} | 숫자 가정 현실화를 위한 이론 교재")

    add_heading(doc, "목적", 1)
    add_para(doc, "이 문서는 LLM token capacity simulation의 숫자를 더 현실적으로 만들기 위한 배경 이론 교재입니다. 회사별 내부 allocation을 단정하지 않고, 공개자료와 논문을 기반으로 물리적/운영적/경제적으로 말이 되는 가정을 만드는 것이 목표입니다.")

    add_heading(doc, "한 문장 요약", 1)
    add_para(doc, "계약 전력은 token capacity가 아니며, 실제 compute 배분은 전력 인프라, accelerator cluster, scheduler, training/inference workload, 제품 SLA, business priority가 겹쳐 결정됩니다.")
    ...
    add_heading(doc, "Long-form Research Report", 1)
    add_para(doc, "아래 장은 요약형 학습 노트가 아니라 조사기관 리포트처럼 긴 본문으로 작성한 해설입니다. 각 장은 공개자료가 직접 말하는 것과, 우리가 시뮬레이션에서 추정해야 하는 것을 구분하는 데 초점을 둡니다.")
    for title_text, paragraphs, takeaways in LONGFORM_CHAPTERS:
        add_heading(doc, title_text, 2)
        for para in paragraphs:
            add_para(doc, para)
        add_para(doc, "핵심 takeaways")
        for takeaway in takeaways:
            doc.add_paragraph(takeaway, style="List Bullet")

    add_heading(doc, "핵심 개념 사전", 1)
    glossary = [
        ("Contracted / planned power", "계약, 계획, 확보, 또는 발표된 전력. active service capacity가 아님."),
        ("Active power", "실제로 전력 인입과 설비 준비가 끝나 운영 가능한 facility power."),
        ("IT load", "PUE를 제거한 서버, storage, network, accelerator 측 전력."),
    ...
    table.rows[0].cells[0].text = "개념"
    table.rows[0].cells[1].text = "의미"
    for term, meaning in glossary:
        row = table.add_row().cells
        row[0].text = term
        row[1].text = meaning

    add_heading(doc, "공부 질문", 1)
    questions = [
    ...
    add_heading(doc, "읽기 자료 요약", 1)
    for sid, title_text, publisher, dt, url, use in SOURCES:
        add_heading(doc, f"{sid}. {title_text}", 2)
        add_para(doc, f"발행자: {publisher} | 날짜: {dt}")
        add_para(doc, f"링크: {url}")
        add_para(doc, f"읽는 목적: {use}")
        add_para(doc, "기록 방식: 원문 숫자 1-3개, 핵심 동사, 우리 모델에서 바뀔 수 있는 assumption_id를 source_review_log.md에 남깁니다.")

    add_heading(doc, "저작권 메모", 1)
    add_para(doc, "뉴스와 유료 리포트 전문은 이 문서에 복사하지 않습니다. 대신 핵심 숫자, 출처, 날짜, 해석, 적용 가능한 assumption만 요약합니다. 논문과 공식 보고서도 필요한 범위에서 요약하고 원문 링크를 남깁니다.")
...
def main() -> None:
    ...
    md_path = DOCS / "llm_compute_power_theory_reader.md"
    docx_path = OUT / "llm_compute_power_theory_reader_kr.docx"
    md_path.write_text(markdown_text, encoding="utf-8")
    build_docx(docx_path, markdown_text)
