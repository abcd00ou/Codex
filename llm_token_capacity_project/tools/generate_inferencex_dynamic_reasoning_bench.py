                ...
                output_rows.append(
                    {
                        ...
                        "users_per_gpu_at_interactivity": scenario_output_tps_gpu
                        / tok_s_user,
...
def build_summary_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for key, items in grouped.items():
        ...
        base_values = [fnum(item["base_output_tok_s_gpu"]) for item in items]
        scenario_values = [fnum(item["scenario_output_tok_s_gpu"]) for item in items]
        users_values = [fnum(item["users_per_gpu_at_interactivity"]) for item in items]
        request_values = [fnum(item["requests_s_gpu_by_osl"]) for item in items]
        effective_multiplier = fnum(items[0]["effective_call_multiplier"])
        summary_rows.append(
            {
                "model": model,
                "gpu": gpu,
                ...
                "base_output_tok_s_gpu_p50": percentile(base_values, 0.50),
                "scenario_output_tok_s_gpu_p50": percentile(scenario_values, 0.50),
                "scenario_output_tok_s_gpu_p10": percentile(scenario_values, 0.10),
                "scenario_output_tok_s_gpu_p90": percentile(scenario_values, 0.90),
                "users_per_gpu_p50": percentile(users_values, 0.50),
                "requests_s_gpu_by_osl_p50": percentile(request_values, 0.50),
...
def write_readme(path: Path, detail_rows: list[dict[str, Any]], summary_rows: list[dict[str, Any]]) -> None:
    ...
    headline = [
        ...
        and row["interactivity_tok_s_user"] in (30, 50, 70)
    ...
    for row in headline:
        table_lines.append(
            "| {model} | {gpu} | {isl}/{osl} | {tok_s_user} | {calls} | {base} | {scenario} | {users} |".format(
                model=row["model"],
                gpu=row["gpu"],
                isl=row["isl"],
                osl=row["osl"],
                tok_s_user=row["interactivity_tok_s_user"],
                calls=fmt(fnum(row["effective_call_multiplier"])),
                base=fmt(fnum(row["base_output_tok_s_gpu_p50"])),
                scenario=fmt(fnum(row["scenario_output_tok_s_gpu_p50"])),
                users=fmt(fnum(row["users_per_gpu_p50"])),
            )
        )
...
def select_rows(
    ...
    *,
    ...
    return [
        ...
        and int(row["interactivity_tok_s_user"]) == interactivity_tok_s_user
        and int(row["isl"]) == isl
        and int(row["osl"]) == osl
    ]
...
def markdown_model_gpu_table(rows: list[dict[str, Any]]) -> str:
    ...
    for row in sorted(
        ...
        key=lambda item: (
            ...
            item["gpu"],
            item["workload_type"],
            int(item["interactivity_tok_s_user"]),
        ),
    ):
        workload = "CoT" if row["workload_type"] == "cot" else "Agentic"
        lines.append(
            "| {workload} | {model} | {gpu} | {framework} | {precision} | {tok_s_user} | {rows} | {scenario} | {users} | {req} |".format(
                workload=workload,
                model=row["model"],
                gpu=row["gpu"],
                ...
                scenario=fmt(fnum(row["scenario_output_tok_s_gpu_p50"])),
                users=fmt(fnum(row["users_per_gpu_p50"])),
                req=fmt(fnum(row["requests_s_gpu_by_osl_p50"])),
            )
        )
    return "\n".join(lines)
...
def markdown_interactivity_table(rows: list[dict[str, Any]]) -> str:
    ...
    for row in ordered:
        key = (row["workload_type"], int(row["interactivity_tok_s_user"]))
        if key in seen:
            continue
        seen.add(key)
...
def sequence_median_rows(rows: list[dict[str, Any]], workload_type: str) -> list[dict[str, Any]]:
    ...
    for isl, osl in [(1024, 1024), (8192, 1024), (1024, 8192)]:
        subset = select_rows(
            rows,
            workload_type=workload_type,
            interactivity_tok_s_user=50,
            isl=isl,
            osl=osl,
        )
        if not subset:
            continue
        scenario_values = sorted(fnum(row["scenario_output_tok_s_gpu_p50"]) for row in subset)
        users_values = sorted(fnum(row["users_per_gpu_p50"]) for row in subset)
        output.append(
            {
                "workload_type": workload_type,
                "isl": isl,
