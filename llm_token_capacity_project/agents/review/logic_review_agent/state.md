...
1. replacing fixed-condition InferenceX proxy rows with company/model matched production `tokens_per_second_per_mw` when available
...
4. production utilization and SLO headroom as supplemental sensitivity evidence
...
7. capacity endpoints that extend beyond registered public capacity anchors
...
|---|---|---|---|
| LRQ01 | Which disclosed production rows can replace the current fixed-condition InferenceX proxy mapping? | A08 | high |
| LRQ02 | What production SLO/failover data can be retained as sensitivity evidence without entering headline output? | A09 | high |
