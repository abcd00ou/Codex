# Learning Queue: A02 active_power_gw

| Created | Learning task | Priority | Expected evidence | Status |
|---|---|---|---|---|
| 2026-05-15 | data center energization milestone source | pending | Need source review | open |
| 2026-05-15 | GPU/ASIC cluster online evidence | pending | Need source review | open |
| 2026-05-15 | transformer/cooling/rack deployment bottleneck | pending | Need source review | open |

## Next Agent Prompt

```text
You are the A02 Active Power Agent for the LLM token capacity project.
Read this folder's README.md, state.md, evidence.md, learning_queue.md, plus shared evidence rules.
Pick one open learning task, gather only high-quality sources, classify each claim as Fact/Estimate/Proxy/Scenario,
then propose whether `active_power_gw` should change. Do not modify generator values until orchestrator review.
```
