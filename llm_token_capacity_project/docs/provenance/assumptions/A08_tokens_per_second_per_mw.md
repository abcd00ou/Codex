...
| reference_serving_tps_per_mw | fleet_reference_tps_per_mw * commercial_workload_fit_factor | This is the coefficient used for headline token generation after each GPU's short chat, long chat and agentic TPS/MW are weighted and then combined by fleet mix. | Provider production output-token throughput with comparable workload/SLO. |
